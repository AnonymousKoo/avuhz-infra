...
- AUTH v30 is prepared as the corrected synthetic-token validation boundary. Plan `d89b1ba0-6a85-4897-8d40-33af4de45e4a`, digest `sha256:8b1d92022e192c73b856d5d399843a3ee4c9dac4d08e9c7778293f765f8ebd6f`, and window `2026-09-15T23:00:00Z` through `2026-09-16T05:00:00Z` are bound, but no owner approval exists and progress remains pristine/unconsumed. The design requires an existing-user recovery link, a separately bound publishable key, immediate local session revocation, zero-session/refresh-token readback, and then exact ES256 issuer/audience/tenant/subject/read-only-policy validation.
...
This is intentional. AUTH v21/v24/v26/v28 completion, AUTH v30 preparation, and DATA v3 completion do not themselves authorize runtime credential creation, secret retrieval, synthetic token issuance, hosted adapter wiring, or readiness promotion. AUTH v30 still requires its own separate exact owner approval and fresh environment-bound credential lifecycle before execution. The default Supabase `aud=authenticated` is rejected by Avuhz; the approved DEVELOPMENT command-service audience remains separate.
...
AUTH v30 is the current prepared forward-only boundary. Plan id `d89b1ba0-6a85-4897-8d40-33af4de45e4a`, plan digest `sha256:8b1d92022e192c73b856d5d399843a3ee4c9dac4d08e9c7778293f765f8ebd6f`, pristine progress digest `sha256:ed6c3afa8aaed2f5f62431c0b6d0976ab35bde08dc7d9a0651725d33e15d7241`, and immutable window `2026-09-15T23:00:00Z` through `2026-09-16T05:00:00Z` are repository-bound. There is no v30 approval and no execution evidence. Step 1 generates one existing-user-only recovery link with a fresh Auth-admin secret; Step 2 consumes it once, creates exactly one short-lived session, immediately signs out that local session, and proves zero sessions/refresh tokens; Step 3 validates the still-live in-memory access JWT against live JWKS and the exact server allowlist after cleanup. Publishable-key material is supplied through the DEVELOPMENT environment boundary rather than enumerated through the Management API.

## Next task

AUTH v26 and AUTH v28 are complete and consumed, and the DEVELOPMENT Auth-admin bootstrap credential is retired. AUTH v27 remains intentionally unconsumed because its narrower boundary did not cover the dashboard bundle. AUTH v29 was rejected before canonicalization. AUTH v30 is prepared but grants no authority until a separate exact owner approval is canonical. The hosted identity resolver remains deliberately unwired.

The intended sequence is now:

1. create and merge the separate exact AUTH v30 owner approval while the immutable window is still usable;
2. only under that approval and fresh DEVELOPMENT environment-bound credentials, execute the one recovery-link -> one session -> immediate local revocation -> post-revocation JWT/policy validation lifecycle and record sanitized outcome evidence;
3. plan and wire the hosted DEVELOPMENT trusted-identity adapter; and only then
4. plan and wire the hosted DEVELOPMENT DATA adapter and promote readiness after both dependencies verify independently.
...
- Do not execute AUTH v30 or issue a synthetic access token until the separate exact v30 owner approval is canonical and fresh v30 environment-bound credentials are available; never retain token material.
...
- AUTH v29 was rejected before canonicalization and has no provider effects or reusable authority. AUTH v30 is prepared/pristine with no approval or execution evidence; its safe path uses recovery rather than magic-link signup semantics and a pre-bound publishable key rather than Management API key enumeration.
...
`REMOTE_AUTHORIZATION`: AUTH v21, v24, v26, and v28 are completed/consumed historical authority. AUTH v22 is expired and unexecuted; AUTH v23 is unapproved and unexecuted; AUTH v25 has expired exact approval but remained unexecuted/unconsumed. AUTH v27 has exact owner approval but remains pristine/unconsumed and cannot authorize the broader dashboard bundle. AUTH v28 is consumed and grants no further execution authority. AUTH v29 never became canonical and grants no authority. AUTH v30 is `READY_FOR_APPROVAL` with pristine/unconsumed progress and no approval artifact; it grants no provider authority yet.

Synthetic token execution under v30, DATA access, Render changes, staging, production, and hosted adapter wiring remain separate boundaries and require their own exact authorization where applicable.
