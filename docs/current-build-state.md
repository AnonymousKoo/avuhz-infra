...
- Do not use the narrower AUTH v27 approval for the dashboard Create hook action. Do not mutate Supabase AUTH until AUTH v28 receives its separate exact owner approval, its approved window is active, and a fresh exact preflight still matches the bound bundle. DATA, Render, n8n, staging, and production remain outside that boundary.
...
- The custom access-token hook function exists and the hosted hook remains unconfigured. AUTH v27 is approved but unconsumed after the dashboard bundle mismatch; AUTH v28 is prepared for the exact observed dashboard bundle but remains unapproved and unexecuted.
...
`REMOTE_AUTHORIZATION`: AUTH v21, v24, and v26 are completed/consumed historical authority. AUTH v22 is expired and unexecuted; AUTH v23 is unapproved and unexecuted; AUTH v25 has expired exact approval but remained unexecuted/unconsumed. AUTH v26 grants no further execution authority. AUTH v27 has exact owner approval but remains pristine/unconsumed and cannot authorize the broader dashboard Create hook bundle observed during preflight. AUTH v28 is `READY_FOR_APPROVAL`, pristine/unapproved/unexecuted, and grants no provider authority.

No provider mutation is currently authorized. Exact v28 dashboard-bundle execution requires its own separate owner approval, active window, and fresh matching preflight; token issuance, DATA access, Render changes, staging, production, and hosted adapter wiring remain separate future boundaries.
