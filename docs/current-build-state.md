...
- AUTH v30 remains immutable historical preparation. AUTH v31 is stopped/consumed and non-retryable. AUTH v32 is the current corrected synthetic-token preparation with the same bounded recovery-link, publishable-key, immediate local-revocation, zero-session/refresh-token readback, and exact JWT/policy-validation scope.
...
This is intentional. AUTH v21/v24/v26/v28 completion, AUTH v31 failure recording, AUTH v32 preparation, and DATA v3 completion do not themselves authorize runtime credential creation, secret retrieval, synthetic token issuance, hosted adapter wiring, or readiness promotion. AUTH v32 requires separate exact approval and execution authorization before credential use. The default Supabase `aud=authenticated` is rejected by Avuhz; the approved DEVELOPMENT command-service audience remains separate.
...
AUTH v32 is the current prepared forward-only boundary. Plan id `e8cd574a-a532-4c95-ab5d-5c069c1bbb96`, plan digest `sha256:c43b0b906bfb0c03e763d6cc5a13d47a900b40eb4db8fd904c799c524ec6c37c`, pristine progress digest `sha256:7539fdec222abaddce8fc6fce1ffd5299c793db34aa97a05932adccc253825f3`, and immutable window `2026-09-17T15:00:00Z` through `2026-09-17T21:00:00Z` are repository-bound. No v32 approval or execution evidence exists. The existing v32 Supabase Secret key and GitHub environment secret are referenced by name only and were not read, changed, rotated, exposed, or used during preparation.

## Next task

AUTH v26 and AUTH v28 are complete and consumed, and the DEVELOPMENT Auth-admin bootstrap credential is retired. AUTH v27 remains intentionally unconsumed, AUTH v29 was rejected before canonicalization, AUTH v30 remains unapproved/unexecuted, and AUTH v31 is stopped/consumed with retry unauthorized. AUTH v32 is prepared but grants no authority until a separate exact owner approval is canonical. The hosted identity resolver remains deliberately unwired.

The intended sequence is now:

1. create and merge the separate exact AUTH v32 owner approval before its immutable effective time;
2. only under that approval and a separate exact execution authorization, use the existing v32 environment-bound credential to execute the one recovery-link -> one session -> immediate local revocation -> post-revocation JWT/policy validation lifecycle and record sanitized outcome evidence;
...
- Do not retry AUTH v31. Do not execute AUTH v32 or issue a synthetic access token until the separate exact v32 owner approval and execution authorization are canonical; never read or use the v32 credential during preparation, and never retain token material.
...
- AUTH v29 was rejected before canonicalization and has no provider effects or reusable authority. AUTH v30 remains pristine/unapproved/unexecuted. AUTH v31 is stopped/consumed and non-retryable. AUTH v32 is prepared/pristine with no approval or execution evidence and pins the corrected recovery-link parser.
