...
1. obtain owner review and, if desired, separately create the exact approval for pristine `development-auth-provider-read-diagnostic-v2` before its effective time; do not approve or execute v1, execute v2, or retry the consumed inspection under preparation authority;
2. preserve the unresolved session/refresh state until a fresh forward-only inspection boundary is separately designed and authorized;
