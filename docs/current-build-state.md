...
`development-auth-v32-session-attribution-owner-interactive-v1` is a separate fresh owner-interactive `PROVIDER_READ` boundary for attributing session state without reading raw rows. Plan `4962f8b6-dad1-45d3-b020-2d60d41409c3`, plan digest `sha256:6977cafc9894894dbb28e0f0f6c8fd3bcc609cdd014eba4144b1d335c4928c7a`, pristine progress digest `sha256:481bfb127db17518dafb21b780b6cf0904099a3e7c8420be8f3f141e8e03b027`, exact approval `ef85b6ea-3b5d-44f4-bd04-0fd71a31a0ba` with digest `sha256:df3964782c3c7c28b9b79b677aaa0fd9ec9b99f2377ac0ae296276b3b6c46c27`, and immutable window `2026-09-22T15:00:00Z` through `2026-09-22T21:00:00Z` bind exactly one Dashboard SQL Editor aggregate SELECT. It returns only `session_count` and `synthetic_session_count`; the canonical synthetic `example.invalid` address is used only as a fixed predicate and is not returned. Results classify as `SYNTHETIC_SESSION_ATTRIBUTED` only for 1/1, `SESSION_DISAPPEARED_REVIEW_REQUIRED` when total sessions are zero, and `SESSION_ATTRIBUTION_MISMATCH_REVIEW_REQUIRED` otherwise. Refresh-token reads, further SQL, raw rows, identifiers, cleanup, issuance, and mutation are prohibited. Progress remains `NOT_STARTED / PENDING / unconsumed`; no execution progress, evidence, workflow, executor, SQL execution, or provider contact exists.
...
1. during its exact active window and only under a separate execution instruction, perform the approved one-query owner-interactive read for `development-auth-v32-session-attribution-owner-interactive-v1`; do not execute the earlier owner-interactive inspection, either provider-read diagnostic, or retry the consumed inspection;
2. preserve the unresolved session/refresh state unless the separately approved owner-interactive attribution query establishes a canonical sanitized result;
