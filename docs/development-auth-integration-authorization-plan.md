...
GitHub Actions run `34449588755`, using the workflow `DEVELOPMENT AUTH Plan v7 PostgreSQL Certification`, concluded `SUCCESS` for exact candidate commit `c27e399c2b7b7aa51255701a9a1bdfe9593a7908` in a disposable PostgreSQL 17 environment. The observed PostgreSQL version was 17.11. The exact v7 bootstrap, hook, and seal migration digests bound above therefore have fresh digest-specific PostgreSQL-17 certification:

- `CURRENT_BOOTSTRAP_PG17_CERTIFIED=YES`
- `CURRENT_HOOK_PG17_CERTIFIED=YES`
- `CURRENT_SEAL_PG17_CERTIFIED=YES`

The run successfully completed exact candidate identity and 15-file scope verification, pinned PostgreSQL image and RepoDigest verification, stable PostgreSQL readiness, the bounded authorization-plan validator, the migration-identity bootstrap PostgreSQL suite, the dedicated NOSUPERUSER `SET ROLE` regression, the hook effective-role PostgreSQL suite, the effective-role privilege-order regression, the migration-identity seal PostgreSQL suite, the baseline gate, diff hygiene, final checkout integrity, and verified disposable PostgreSQL cleanup. No PostgreSQL-backed migration test was skipped. Successful hook certification also proves that the superseded `ACL arrays must be one-dimensional` defect did not preempt the intended effective-role privilege-order regression.

The preceding natural run `34446391662` remains historical failed harness evidence classified as `CI_POSTGRES_STARTUP_READINESS_RACE`; it ended before the Avuhz validator or any PostgreSQL-backed Avuhz certification suite executed. Run `34449588755` supersedes that harness failure for the current migration-artifact certification truth.

Run `34449588755` certifies exact candidate commit `c27e399c2b7b7aa51255701a9a1bdfe9593a7908`. This documentation edit occurs after that run. Until the documentation correction is committed, pushed, and naturally revalidated, no future commit containing this edit is claimed to have passed CI. The migration artifacts themselves are unchanged and retain their exact-digest PostgreSQL-17 certification. CI certification is not provider execution, approval, authorization, deployment, hook enablement, or production readiness.

## Exact ordered sequence

| Step | Class | Exact resource/change | Allowed credential class | Required gate and postcondition |
|---|---|---|---|---|
| 1 | Local-only | Validate the ordered bootstrap, effective-role hook, and immediate seal artifacts | `NONE` | Current v7 local evidence binds the corrected bootstrap, hook, seal, and focused tests; the exact current bootstrap/hook/seal digests passed fresh PostgreSQL-17 certification in run `34449588755`; no provider migration has been applied, no hook has been created or enabled remotely, and Step 1 provider execution remains not started |
...
The historical v2 evidence remains immutable lineage. The rejected, uncommitted v3/v4 drafts are superseded and are not current artifacts. The v5 evidence at `contracts/plans/v1/development-auth-step1-v5-local.evidence.json`, canonical digest `sha256:8d73fcd2b1ae9475a213716b8cd314803c5215b6b17eaa99dcb37d0e06d8a07e`, is preserved only as historical failed-certification lineage. The v6 evidence is preserved only as rejected pre-commit lineage. Current v7 local evidence remains the plan-bound local/pre-CI evidence; it binds the forward-corrected artifacts without claiming provider execution. Later run `34449588755` supplies the fresh PostgreSQL-17 certification for those exact unchanged migration digests.

The migration identity binding and authorization window remain unresolved, and no approval exists. Provider-side exact function-state verification, the synthetic DEVELOPMENT Auth identity, provider-assigned subject, dedicated DEVELOPMENT tenant UUID, server-policy/capability binding, hook configuration reference, and ephemeral token/JWKS/local-validation evidence remain unresolved where applicable. The hook remains disabled, and no provider step has begun.
...
Every provider step requires a new exact authorization after its prior evidence exists; any binding change requires a new plan version and digest. PostgreSQL-17 certification run `34449588755` does not resolve any provider binding, create approval, open an authorization window, or begin provider execution.
