...
When the future approval-recording command succeeds, `human_approval.recorded` records the non-authoritative lifecycle fact for the target diagnostic scope. It cannot establish authority on its own: the separate HumanApproval record remains the only authoritative approval evidence.
