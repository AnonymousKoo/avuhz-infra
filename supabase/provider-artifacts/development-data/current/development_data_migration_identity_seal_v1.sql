...
-- This seal removes temporary CREATEROLE, public-schema DDL/delegation authority, and the
-- explicit postgres SET edge. Provider-imposed ADMIN/NOSET/NOINHERIT edges are verified
-- exactly and may remain because hosted non-superuser CREATEROLE semantics create them.
...
begin
  ...
  if provider_edge_count not in (0, 1)
     ...
     or total_membership_edge_count <>
       provider_edge_count + explicit_set_edge_count + command_admin_edge_count then
  ...
  if (
    ...
    cross join lateral aclexplode(coalesce(namespace.nspacl, '{}'::aclitem[])) schema_acl
     where namespace.nspname = 'public'
       and schema_acl.grantor = migration_role_oid
       and schema_acl.grantee = command_role_oid
       and schema_acl.privilege_type = 'USAGE'
       and not schema_acl.is_grantable
  ) <> 1
     or exists (
       ...
       cross join lateral aclexplode(coalesce(namespace.nspacl, '{}'::aclitem[])) schema_acl
        where namespace.nspname = 'public'
          and schema_acl.grantor = migration_role_oid
          and not (
            schema_acl.grantee = command_role_oid
            and schema_acl.privilege_type = 'USAGE'
            and not schema_acl.is_grantable
          )
     ) then
    raise exception 'Avuhz DEVELOPMENT DATA pre-seal delegated schema privilege mismatch';
  end if;
  ...
  if exists (
    ...
    raise exception 'Avuhz DEVELOPMENT DATA migration did not execute under the bounded migration identity';
  end if;
...
-- Preserve the runtime schema privilege through the owner before removing the temporary
-- grant-option chain created by the migration identity.
...
revoke usage on schema public from avuhz_data_migration_service_dev cascade;
...
begin
  ...
  if provider_edge_count not in (0, 1)
     or command_admin_edge_count <> 1
     or total_membership_edge_count <> provider_edge_count + command_admin_edge_count
     or exists (
  ...
  if exists (
    ...
    raise exception 'Avuhz DEVELOPMENT DATA sealed migration identity retains direct or delegated public schema privilege';
  end if;

  if not has_schema_privilege('avuhz_command_service', 'public', 'USAGE') then
    raise exception 'Avuhz DEVELOPMENT DATA sealed command-service role lost public schema usage';
  end if;
