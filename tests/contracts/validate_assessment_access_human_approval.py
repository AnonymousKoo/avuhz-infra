...
ROOT=Path(__file__).resolve().parents[2];SC=ROOT/"contracts/schemas/v1";SID="urn:avuhz:schema:contracts:domain:human-approval:v1";FIX=ROOT/"contracts/fixtures/v1/assessment-access-human-approval.cases.json";OLD=ROOT/"contracts/fixtures/v1/approval-diagnostic-scope.cases.json"
def load(p):
...
def main():
 ...
 if s["properties"]["subject_type"]["enum"]!=["DIAGNOSTIC_SCOPE","ASSESSMENT_ACCESS_PROPOSAL","OIA_CONVERSION_DECISION","ONGOING_AGREEMENT_AUTHORITY","ONGOING_ACCESS_GRANT","IMPLEMENTATION_BRIEF","IMPLEMENTATION_AUTHORIZATION","CODEX_BUILD_PACKAGE","DEPLOYMENT_AUTHORIZATION"]:fail("closed subject vocabulary drifted")
