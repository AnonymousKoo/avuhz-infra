...
PAYLOAD_IDS = {
    ...
    "RecordHumanApproval": "urn:avuhz:schema:contracts:commands:record-human-approval-payload:v1",
    "ApproveDiagnosticScope": "urn:avuhz:schema:contracts:commands:approve-diagnostic-scope-payload:v1",
    "CanonicalizeDiagnosticScope": "urn:avuhz:schema:contracts:commands:canonicalize-diagnostic-scope-payload:v1",
}
SUBJECTS = {"AcceptAcquisitionHandoff": "ACQUISITION_HANDOFF", "OpenEngagement": "ENGAGEMENT", "SubmitDiagnosticScope": "ENGAGEMENT", "RecordHumanApproval": "DIAGNOSTIC_SCOPE", "ApproveDiagnosticScope": "DIAGNOSTIC_SCOPE", "CanonicalizeDiagnosticScope": "DIAGNOSTIC_SCOPE"}
...
def envelope(command, payload):
    ...
    if command in ("OpenEngagement", "SubmitDiagnosticScope", "RecordHumanApproval", "ApproveDiagnosticScope", "CanonicalizeDiagnosticScope"):
        value["engagement_id"] = "a3000000-0000-4000-8000-000000000004"
    if command in ("SubmitDiagnosticScope", "RecordHumanApproval", "ApproveDiagnosticScope", "CanonicalizeDiagnosticScope"):
...
def composed(command):
    ...
    if command in ("SubmitDiagnosticScope", "RecordHumanApproval", "ApproveDiagnosticScope", "CanonicalizeDiagnosticScope"): constraints["required"] += ["engagement_id", "expected_record_version"]
...
def main():
    ...
    if len(fixtures["positive"]) != 6 or len(fixtures["negative"]) != 55: fail("fixture inventory drifted")
