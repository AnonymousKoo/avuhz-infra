...
SUBJECTS = {"AcceptAcquisitionHandoff": "ACQUISITION_HANDOFF", "OpenEngagement": "ENGAGEMENT", "SubmitDiagnosticScope": "ENGAGEMENT", "ApproveDiagnosticScope": "DIAGNOSTIC_SCOPE", "CanonicalizeDiagnosticScope": "DIAGNOSTIC_SCOPE"}
...
def envelope(command, payload):
    ...
    if command in ("OpenEngagement", "SubmitDiagnosticScope", "ApproveDiagnosticScope", "CanonicalizeDiagnosticScope"):
        value["engagement_id"] = "a3000000-0000-4000-8000-000000000004"
    if command in ("SubmitDiagnosticScope", "ApproveDiagnosticScope", "CanonicalizeDiagnosticScope"):
...
def composed(command):
    ...
    if command in ("SubmitDiagnosticScope", "ApproveDiagnosticScope", "CanonicalizeDiagnosticScope"): constraints["required"] += ["engagement_id", "expected_record_version"]
...
def main():
    ...
    if len(fixtures["positive"]) != 5 or len(fixtures["negative"]) != 47: fail("fixture inventory drifted")
