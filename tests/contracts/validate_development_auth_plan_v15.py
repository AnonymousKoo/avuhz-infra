#!/usr/bin/env python3
"""Validate DEVELOPMENT AUTH v15 through Step 2 success persistence."""
...
APPROVAL_DIGEST_EXPECTED = APPROVAL_DIGEST
...
def main() -> None:
    ...
    for moment in (WINDOW_START, STEP1_AUTH_AT, STEP1_OUTCOME_AT, STEP2_AUTH_AT, STEP2_OUTCOME_AT):
    ...
    assert approval["approval_digest"] == APPROVAL_DIGEST_EXPECTED
    ...
    step1, step2, _, step4 = plan["steps"]
    ...
    step1_authorized = authorize_step(
        plan, approval, progress, step1_request, SCHEMA_ROOT, STEP1_AUTH_AT,
    ...
    assert canonical_digest("postgres.role.avuhz_migration_service_dev") == MIGRATION_IDENTITY_VALUE_DIGEST
    step1_success = record_step_outcome(
        plan, approval, step1_authorized, STEPS[0], "SUCCEEDED", "PASS",
        step1_outcome_evidence, step1["expected_postcondition"], None, SCHEMA_ROOT,
        STEP1_OUTCOME_AT, binding_assertions=[step1_binding],
    )
    ...
    step2_request["required_evidence"] = [
        {"evidence_type": "migration.identity.v3.bootstrap.verified", "evidence_digest": STEP1_EVIDENCE_DIGEST},
        {"evidence_type": "hook.migration.v2.artifact.certified", "evidence_digest": artifacts[HOOK]},
    ]
    ...
    assert step2_evidence_file["provider_observation"]["hook_owner"] == "avuhz_migration_service_dev"
    ...
    expected_step2_success = record_step_outcome(
        plan, approval, step2_authorized, STEPS[1], "SUCCEEDED", "PASS",
        step2_outcome_evidence, step2["expected_postcondition"], None, SCHEMA_ROOT,
        STEP2_OUTCOME_AT, binding_assertions=[step2_binding],
    )

    assert execution == expected_step2_success
    assert execution["record_version"] == 5
    assert execution["overall_state"] == "IN_PROGRESS"
    assert execution["updated_at"] == STEP2_OUTCOME_AT
    assert execution["progress_digest"] == STEP2_SUCCESS_PROGRESS_DIGEST
    ...
    assert len(step2_state["evidence"]) == 1
    assert len(step2_state["binding_assertions"]) == 2
    ...
    print(
        ...
        "Step 2 exact hook-v2 evidence persisted; Steps 3-4 pending; "
        "hook enablement unauthorized/unperformed; DEVELOPMENT DATA untouched)"
    )
