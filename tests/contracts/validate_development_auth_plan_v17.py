#!/usr/bin/env python3
"""Validate the repository-only DEVELOPMENT AUTH v17 synthetic-identity plan package."""
...
PLAN_PATH = ROOT / "contracts/plans/v1/development-auth-integration-v17.plan.json"
PROGRESS_PATH = ROOT / "contracts/plans/v1/development-auth-integration-v17.progress.json"
...
PLAN_ID = "a96bff23-3de1-4cb9-b82d-173ef51c3031"
PROGRESS_ID = "b76e9c50-205b-4544-8269-0c3261417c55"
PLAN_DIGEST = "sha256:58da83d494d2efdd19b09a5ac837b0804be61df24dd88abc0a1ed0c51dcf85d2"
PROGRESS_DIGEST = "sha256:1da351979f0875f3c1c1eb1cb41a48c7748b394f2c151b1ce39d30e2526aff50"
...
STEP_ID = "development.auth.v17.step.01.create-synthetic-identity"
CREATED_AT = "2026-09-14T05:40:45Z"
WINDOW_START = "2026-09-14T06:00:00Z"
WINDOW_END = "2026-09-14T09:00:00Z"
...
def main() -> int:
    ...
    preapproval = next(
        ...
        if item["binding_id"] == "binding.development.auth.v17.v16-disabled-hook-state"
    )
    ...
    preflight = next(
        item for item in step["binding_declarations"]
        if item["binding_id"] == "binding.development.auth.v17.identity-create-preflight"
    )
    ...
    produced = next(
        item for item in step["binding_declarations"]
        if item["binding_id"] == "binding.development.auth.v17.synthetic-identity"
    )
    ...
    assert step["produced_evidence"] == [{
        ...
        "established_binding_ids": ["binding.development.auth.v17.synthetic-identity"],
    ...
    v16_state = v16_execution["step_states"][0]
    assert v16_state["authorization_state"] == "CONSUMED"
    assert v16_state["execution_state"] == "SUCCEEDED"
    assert v16_state["verification_state"] == "PASS"
    v16_binding = next(
        item for item in v16_state["binding_assertions"]
        if item["binding_id"] == "binding.development.auth.v16.hook-deployed-state"
    )
    assert v16_binding["evidence_digest"] == V16_EVIDENCE_DIGEST
    assert v16_binding["value_digest"] == V16_DEPLOYED_STATE_VALUE_DIGEST
    ...
    for path in (
        ROOT / "contracts/plans/v1/development-auth-integration-v17.approval.json",
        ROOT / "contracts/plans/v1/development-auth-integration-v17.execution-progress.json",
        ROOT / "contracts/plans/v1/development-auth-step1-v17-preflight.evidence.json",
        ROOT / "contracts/plans/v1/development-auth-step1-v17-success.evidence.json",
    ):
        assert not path.exists(), path

    print(
        "DEVELOPMENT_AUTH_V17_PLAN=PASS "
        "(one synthetic identity only; v16 disabled-hook evidence bound; "
        "Step 1 pending/unexecuted/unconsumed; no approval; provider mutation not attempted)"
    )
    return 0
