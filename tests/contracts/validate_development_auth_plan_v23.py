#!/usr/bin/env python3
"""Validate the fresh DEVELOPMENT AUTH v23 continuation before execution."""
...
PLAN_PATH = BASE / "development-auth-integration-v23.plan.json"
PROGRESS_PATH = BASE / "development-auth-integration-v23.progress.json"
APPROVAL_PATH = BASE / "development-auth-integration-v23.approval.json"
...
WORKFLOW_PATH = ROOT / ".github/workflows/development-auth-v23-tenant-metadata-execution.yml"
READ_ONLY_REFERENCE_PATH = ROOT / ".github/workflows/d4c4d2c-read-only-certification.yml"

PLAN_ID = "f46b60f1-ee75-4cc7-97a9-a3ad361ce708"
PLAN_VERSION = 23
PLAN_DIGEST = "sha256:79e2d712e4b446ad443ec299dba0ddc123b83f6b3e27926b4e31de37be342dc4"
STEP_ID = "development.auth.v23.step.01.bind-synthetic-tenant-app-metadata"
PROJECT = "pwlhruwutoitnieactol"
DATA_PROJECT = "gnuqaefotwgkwurjpyik"
WINDOW_START = "2026-09-14T23:15:00Z"
WINDOW_END = "2026-09-15T02:15:00Z"
...
def main() -> int:
    ...
    for prohibited in (
        ...
        "v22.credential.reuse",
    ):
        assert prohibited in step["prohibited_actions"]
        assert prohibited in plan["prohibited_actions"]

    subject = declaration(step, "binding.development.auth.v23.synthetic-subject-digest")
    ...
    tenant = declaration(step, "binding.development.auth.v23.canonical-tenant")
    ...
    metadata = declaration(step, "binding.development.auth.v23.target-app-metadata")
    ...
    capability = declaration(step, "binding.development.auth.v23.admin-executor-capability")
    ...
    live_preflight = declaration(step, "binding.development.auth.v23.live-read-only-preflight")
    ...
    metadata_preflight = declaration(step, "binding.development.auth.v23.tenant-metadata-preflight")
    ...
    produced = declaration(step, "binding.development.auth.v23.tenant-metadata")
    ...
    assert PROGRESS_PATH.exists(), "v23 progress artifact is required before PR gate"
    ...
    assert WORKFLOW_PATH.exists(), "v23 executor workflow is required before PR gate"
    workflow = WORKFLOW_PATH.read_text(encoding="utf-8")
    required_workflow_fragments = (
        "name: DEVELOPMENT AUTH v23 Tenant Metadata Execution",
        "workflow_dispatch:",
        "EXECUTE_V23_TENANT_METADATA",
        "contents: read",
        "actions: read",
    ...
    for forbidden in (
        BASE / "development-auth-integration-v23.execution-progress.json",
        BASE / "development-auth-step1-v23-preflight.evidence.json",
        BASE / "development-auth-step1-v23-success.evidence.json",
    ):
        assert not forbidden.exists(), forbidden

    print(
        "DEVELOPMENT_AUTH_V23_EXECUTOR_PREPARED=PASS "
        "(fresh forward-only plan, pristine progress, mandatory live read-only preflight, "
        "single bounded metadata mutation, and no v23 execution evidence present)"
    )
    return 0
