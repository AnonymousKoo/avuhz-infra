...
def main():
 ...
 for p in paths:
  ...
  schemas[s["$id"]]=s
 if len(paths)!=144:fail(f"expected 144 schemas, found {len(paths)}")
 ...
 if set(COMMANDS_C)&set(COMMANDS)!=set(COMMANDS_C[:2]):fail("D1 runtime boundary drifted")
 if len(SCHEMA_FILES)!=129:fail("D1 runtime schema boundary drifted")
 approval=schemas[DOMAIN["approval"]]
 if approval["properties"]["actor_role"]["enum"][-2:]!=["CLIENT_DEPLOYMENT_AUTHORITY","SEKINFRA_DEPLOYMENT_AUTHORITY"]:fail("deployment human roles")
 ...
 sekinfra={**common,"approval_id":"d5c00000-0000-4000-8000-000000000010","authority_category":"SEKINFRA_AUTHORITY","actor_identity":"human.sekinfra.deploy","actor_organization":"organization.sekinfra","actor_role":"SEKINFRA_DEPLOYMENT_AUTHORITY","idempotency_key":"phase5dc-sekinfra-approval-0001"}
 valid(DOMAIN["approval"],client,schemas,"client deployment approval"); valid(DOMAIN["approval"],sekinfra,schemas,"Sekinfra deployment approval")
 spoof=copy.deepcopy(client); spoof["actor_role"]="SEKINFRA_DEPLOYMENT_AUTHORITY"; invalid(DOMAIN["approval"],spoof,schemas,"deployment role spoof")
 ...
 print("phase5d-c validation: PASS (4 resources, 9 commands, 8 capabilities, 9 events, 5 read models, 3 industries, security/stale-source negatives, D1 runtime boundary exact)")
