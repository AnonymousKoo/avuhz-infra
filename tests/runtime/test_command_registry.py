...
class CommandRegistryTests(unittest.TestCase):
    def test_registry_is_exactly_slice_one(self):
        self.assertEqual(set(COMMANDS), {"AcceptAcquisitionHandoff", "OpenEngagement", "SubmitDiagnosticScope", "RecordHumanApproval", "ApproveDiagnosticScope", "CanonicalizeDiagnosticScope"})
        self.assertEqual([entry.executable for entry in COMMANDS.values()], [False] * 6)
    ...
    def test_schema_catalog_is_fixed_and_local(self):
        ...
        self.assertEqual(len(registry.schema_ids), 28)
